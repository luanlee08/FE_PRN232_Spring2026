    ...
    const openEditModal = async (address: AddressResponse) => {
        ...
        if (address.city) {
            const matchedProvince = provinces.find(p =>
                p.name === address.city || p.full_name === address.city
            );
            if (matchedProvince) {
                ...
                if (address.district) {
                    const matchedDistrict = districtList.find(d =>
                        d.name === address.district || d.full_name === address.district
                    );
                    if (matchedDistrict) {
                        ...
                        if (address.ward) {
                            const matchedWard = wardList.find(w =>
                                w.name === address.ward || w.full_name === address.ward
                            );
                ...
                {addresses.length === 0 ? (
                    <div className="bg-white rounded-lg p-12 text-center">
                        ...
                        <p className="text-gray-600 mb-6">
                            Thêm địa chỉ để tiện thanh toán nhanh hơn
                        </p>
                        <button
                            onClick={openAddModal}
                                ...
                                <div className="mb-4">
                                    <div className="flex items-start justify-between mb-2">
                                        <div>
                                            ...
                                            <p className="text-gray-600 text-sm mt-1">{address.phoneNumber || 'Không có SĐT'}</p>
                                        </div>
                                    </div>
