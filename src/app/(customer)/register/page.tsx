...
export default function RegisterPage() {
  ...
  const { googleRegister, isLoading: isGoogleLoading } = useGoogleAuth();
  ...
  const triggerGoogleRegister = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      const result = await googleRegister(tokenResponse.access_token);
      if (result.success) {
        refreshUser();
        setTimeout(() => {
          router.push('/');
    ...
    onError: () => {
      setErrors({ google: 'Đăng ký Google thất bại. Vui lòng thử lại.' });
    },
  });
