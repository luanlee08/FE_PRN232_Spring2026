...
function HeaderSearchSync({ onKeywordChange }: { onKeywordChange: (k: string) => void }) {
  const searchParams = useSearchParams();
  const urlKeyword = searchParams.get("keyword") ?? "";
  useEffect(() => {
    onKeywordChange(urlKeyword);
  }, [urlKeyword, onKeywordChange]);
  return null;
}
...
export function Header() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      const timer = setTimeout(() => setCartCount(0), 0);
      return () => clearTimeout(timer);
    }
  ...
  return (
    <>
      <Suspense fallback={null}>
        <HeaderSearchSync onKeywordChange={setSearchKeyword} />
      </Suspense>
